...
<form action="<?php echo str_replace('&', '&amp;', $PG_TXN_URL); ?>" method="post" id="checkout">
	<div class="hiddenFormPGFieldsDiv">
  	...
  	<?php
  		if(isset($PG_PROMOCODE_STATUS) && isset($PG_promocode_LOCAL_VALIDATION) && isset($PG_PROMOCODE_VALUE)){
  			if($PG_PROMOCODE_STATUS=='enabled'){
  				if(($PG_promocode_LOCAL_VALIDATION=='enabled' && trim($PG_PROMOCODE_VALUE)!='') || $PG_promocode_LOCAL_VALIDATION=='disabled'){
  	?>
	  	<div class="form-group">
	  		...
	  		<div class="col-md-8" style="margin-bottom: 5px;">
				<span class="input-group-btn">
	  				<input type="text" id="promoCode" class="form-control pull-left" placeholder="PG Promo Code" style="width: 70%;">
	...
	$(document).ready(function(){
		$('.btnPromoCode').click(function(){
			...
			if($.trim($("#promoCode").val())!=''){
				...
				$.ajax({
					url: 'index.php?rt=extension/pg/applyCode',
					type: 'post',
					dataType: 'json',
					...
					success: function(res){
						...
						if(res.hiddenFields.length > 0){
							$('.hiddenFormPGFieldsDiv').html(res.hiddenFields);		
						}
					}
				});
