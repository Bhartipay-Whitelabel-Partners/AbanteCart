...
class ControllerResponsesExtensionPg extends AController {
	...
	public function main() {
		...
		$this->load->model('extension/pg');
		$this->loadModel('account/customer');
		$CUST_ID=$this->customer->getId();
		$order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		$template_data['PG_MERCHANT_KEY'] = $this->config->get('pg_merchant_key');
		$template_data['PG_PROMOCODE_STATUS'] = $this->config->get('promocode_status');
		$template_data['PG_promocode_LOCAL_VALIDATION'] = $this->config->get('promocode_local_validation');
		$template_data['PG_PROMOCODE_VALUE'] = $this->config->get('promocode_value');
		$template_data['PG_MERCHANT_MID'] = $this->config->get('pg_merchant_mid');
		$template_data['PG_MERCHANT_WEBSITE'] = $this->config->get('pg_merchant_website');
		$template_data['PG_MERCHANT_CALLBACK_URL'] = trim($this->config->get('pg_callback_url'));
		$template_data['PG_TRANSACTION_URL'] = $this->config->get('pg_merchant_transaction_url');
		$template_data['PG_TRANSACTION_STATUS_URL'] = $this->config->get('pg_merchant_transaction_status_url');
		$template_data['PG_CALLBACK'] = $this->config->get('pg_callback');
		$template_data['MID'] = $template_data['PG_MERCHANT_MID'];
        ...
        $template_data['INDUSTRY_TYPE_ID'] = $this->config->get('pg_merchant_industry'); 
        ...
        $template_data['WEBSITE'] = $template_data['PG_MERCHANT_WEBSITE'];      
        
		$PG_DOMAIN = "#";
		if ($template_data['PG_ENVIRONMENT'] == 'live') {
			$PG_DOMAIN = '#';
		}
		
		$template_data['PG_STATUS_QUERY_URL']=$template_data['PG_TRANSACTION_STATUS_URL'];
		$template_data['PG_TXN_URL']=$template_data['PG_TRANSACTION_URL'];
		...
		$template_data['CALLBACK_URL'] =$template_data['PG_MERCHANT_CALLBACK_URL']!=''?$template_data['PG_MERCHANT_CALLBACK_URL']:$this->html->getSecureURL('extension/pg/callback');
		$paramList["CALLBACK_URL"] = $template_data['CALLBACK_URL'];

		$template_data['customCallbackUrl']=$template_data['PG_MERCHANT_CUSTOM_CALLBACKURL'];
		$template_data['callBackUrl']=$template_data['PG_MERCHANT_CALLBACK_URL'];

		//salt
		$salt  = $template_data['PG_MERCHANT_KEY'];
	    //pay id
		$pay_id = $template_data['PG_MERCHANT_MID'];
       //Request Url

		$request_url = $template_data['PG_TXN_URL'];
		...
		$pg_transaction->setReturnUrl($template_data['PG_TRANSACTION_STATUS_URL']);
		...
		$this->processTemplate('responses/pg.tpl' );
	...
	public function callback() {
		...
		//rscorp19
		$PG_MERCHANT_KEY = $this->config->get('pg_merchant_key');

        if (isset($_REQUEST['STATUS']) && ($_REQUEST['STATUS'] == 'Captured')) {
        	$this->load->model('checkout/order');
					$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('pg_order_status_id'));
